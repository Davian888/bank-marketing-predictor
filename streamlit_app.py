...
<style>
    ...
    /* LABEL FIX - Make labels visible with dark color */
...
def create_feature_dataframe(inputs):
    ...
    df['high_balance'] = (df['balance'] > 1300).astype(int)
...
def main():
    ...
    if st.button("🔮 Predict Subscription Likelihood", use_container_width=True):
        # Collect inputs (NO DURATION - removed due to data leakage)
        inputs = {
            'age': age,
            'job': job,
            'marital': marital,
