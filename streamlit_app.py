...
# Custom CSS - FIXED VISIBLE LABELS
st.markdown("""
<style>
    ...
    * { font-family: 'Inter', sans-serif; }
    ...
    /* DARK LABELS - Changed from #ffffff to #1e3a5f for visibility */
    ...
    .section-card h4 { color: #1a1a2e !important; margin: 0; }
    ...
    .main-header h1 { margin: 0; font-size: 2.5em; color: white !important; }
    .main-header p { color: rgba(255,255,255,0.9) !important; margin: 8px 0 0 0; }
    ...
    .result-yes h2, .result-yes p { color: white !important; }
    ...
    .result-no h2, .result-no p { color: white !important; }
    ...
    [data-testid="stMetricValue"] { color: #38ef7d !important; font-size: 24px !important; }
...
def create_feature_dataframe(inputs):
    ...
    # Economic sentiment (normalized between 0-1)
    # cons.conf.idx ranges from about -50.8 to -26.9
...
def main():
    ...
    with st.sidebar:
        st.markdown("## 📊 About")
        st.info("Predicts if a bank client will subscribe to a term deposit based on demographics, campaign data, and economic indicators.")
    ...
    with col4:
        ...
        pdays = st.number_input("Days Since Last Contact", min_value=0, max_value=999, value=999,
                                help="999 = client was never contacted before")
    ...
    if st.button("🔮 PREDICT SUBSCRIPTION", use_container_width=True):
        ...
        try:
            ...
            with col_a:
                ...
                if emp_var_rate < 0:
                    st.markdown("✅ **Negative Employment Rate** - Better economic timing")
                if contact == 'cellular':
            ...
            with col_b:
                ...
                if cons_conf_idx < -45:
                    st.markdown("⚠️ **Low Consumer Confidence** - Timing may not be ideal")
